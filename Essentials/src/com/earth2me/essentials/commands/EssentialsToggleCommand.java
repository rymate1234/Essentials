...
public abstract class EssentialsToggleCommand extends EssentialsCommand
{
	...
	protected Boolean matchArgument(final String arg)
	{
	...
	protected void otherPlayers(final Server server, final CommandSender sender, final String[] args) throws NotEnoughArgumentsException
	{
		...
		for (Player matchPlayer : matchedPlayers)
		{
			...
			if (args.length > 1)
			{
				Boolean toggle = matchArgument(args[1]);
				if (toggle == true)
				{
	...
	abstract void togglePlayer(CommandSender sender, User user, Boolean enabled);
}
